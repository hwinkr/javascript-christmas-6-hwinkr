...
> `Badge` 도메인은 12월 이벤트 배지를 표현합니다. 총 혜택 금액에 따라서 12월 이벤트 배지가 결정 되므로, 배지를 결정하는 것을 `Badge` 도메인 로직으로 구현하기로 했습니다.
...
- [x] `DiscountService`, `Gift`, `Badge`의 도메인 로직을 외부에서 사용할 수 있도록 메서드를 구현한다.
- [x] 아무 혜택을 받지 못하는 경우에도 전체 혜택 내역 정보를 생성한다.
...
아무 혜택을 받지 못할 때도 전체 혜택 정보를 생성한 이유는, 출력을 담당하는 `OutputView`에서 어떻게 출력할지 결정할 수 있도록 하기 위해서 입니다. 만약, 혜택을 받지 못하는 경우에도 위와 같이 모두 0원으로 출력하도록 기획이 변경되더라도 도메인 로직 변경 없이 출력을 결정하는 `OutputView`의 로직만 변경하면 되기 때문에, 더 유연하게 대처할 수 있을것이라 생각했기 때문입니다.

# EventController

> 도메인 로직과 UI로직을 연결하기 위해서 컨트롤러를 구현하기로 했습니다.`EventPlanner`, `InputView`, `OutputView`를 사용해 크리스마스 프로모션 문제를 해결하기 위한 명령을 전합니다.

- [x] `InputView`에 입력을 받아 `EventPlanner`에 전달한다.
- [x] `EventPlanner`가 전달한 문제 해결 결과를 `OutputView`에 전달해 출력하도록 명령을 전달한다.
...
- [x] 주문 메뉴 출력하기
- [x] 할인 전 총주문 금액, 증정 메뉴, 혜택 내역, 총혜택 금액, 할인 후 예상 결제 금액, 12월 이벤트 배지 출력하기
- [x] 증정 메뉴, 혜택 내역을 어떻게 보여줄 지 결정하는 로직 구현하기

- 증정품의 갯수가 0인 경우 '없음'을 출력한다.
- 혜택 내역의 금액이 모두 0인경우 '없음'을 출력한다.

`EventPlanner`가 생성한 혜택 정보를 보여줄 때 제약조건들이 있었습니다.

- 고객에게 적용된 이벤트만 보여준다.
- 총주문 금액이 10,000이하인 경우 이벤트를 적용하지 않는다.

# 기타

- [x] [고객에게 안내할 이벤트 주의 사항](https://github.com/hwinkr/javascript-christmas-6-hwinkr#%EA%B3%A0%EA%B0%9D%EC%97%90%EA%B2%8C-%EC%95%88%EB%82%B4%ED%95%A0-%EC%9D%B4%EB%B2%A4%ED%8A%B8-%EC%A3%BC%EC%9D%98-%EC%82%AC%ED%95%AD)을 어떻게 활용할지 결정하기

1. "총주문 금액 10,000원 이상부터 이벤트가 적용됩니다."

- 총주문 금액이 10,000원 이하인 경우 콘솔에 메시지를 출력

2. 음료만 주문 시, 주문할 수 없습니다

- 주문 유효성 검사에서 사용

3. 메뉴는 한번에 20개까지만 주문할 수 있습니다.
